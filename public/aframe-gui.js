...
AFRAME.registerComponent('gui-button', {
    ...
    update: function update(oldData) {
        //console.log("In button update, toggle: "+this.data.toggleState, oldData);
...
AFRAME.registerComponent('gui-cursor', {
    ...
    init: function init() {
        ...
        el.setAttribute('raycaster','interval', data.interval);
        const aF = data.scaling,
          aT = aF * 1.25,
          // need to look at the timing as this may vary?
          animationScaling = `property: scale; from: ${aF} ${aF} ${aF}; to:${aT} ${aT} ${aT}; dur:200; easing:easeInQuad; startEvents: click`;
        ...
        if (data.design == 'dot') {
            ...
            el.setAttribute('scale',`${aF} ${aF} ${aF}`);
            ...
            if (fuse) {
                var fuseLoader = document.createElement("a-entity");
                fuseLoader.setAttribute('id','fuseLoader');
...
// Reset cursor - what is this, assumes cursor is called cursor, if it is impotant should getElelementsByTagname
var cursor = document.querySelector("#cursor");
if (cursor) {
    cursor.addEventListener("stateremoved", function (evt) {
        if (evt.detail.state === 'cursor-fusing') {
            // more reliable to use el.setAttribute....
